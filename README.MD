...
According to ArgoCD the `best method` is to keep the environments in `a seperate repo`, but we can also add different `Application` resources
in the same project and overlay in different namespaces.